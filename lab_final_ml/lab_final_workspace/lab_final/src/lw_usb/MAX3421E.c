...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	int ret = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, 2);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int retVal = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, nbytes+1);
	//if return code != 0 print an error
	if (retVal != 0){
		xil_printf("Error");
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//read return code from SPI peripheral 
	int retVal = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, 2);
	//if return code != 0 print an error
	if (retVal != 0){
		xil_printf("Error");
	}
